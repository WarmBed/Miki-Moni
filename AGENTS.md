...
> 巫女 (Miki the Monitor)：把多個 VSCode Codex panel session 收進一張本地儀表板，並可選擇加密 relay 到手機。
> 品牌名 **Miki-Moni**（package name `miki-moni`），舊目錄 `~/.cc-hub/` 已於 2026-05-17 自動遷移到 `~/.miki-moni/`。repo 名仍叫 `cc-hub`（還沒 rename）。

## 專案定位

- **痛點驅動**：把散落 Codex session 收進一張儀表板，等回應時喊使用者；不卷功能追其他 repo（hoangsonww/simple10 等）。
...
| `extension/` | `miki-helper` VSCode extension，WS 連回 daemon 處理 `/send`（內部 `Codex-vscode.focus`） |
...
| `hooks/miki-emit.ps1` | Codex hooks（SessionStart / Stop / UserPromptSubmit / PreToolUse / PostToolUse）POST 到 `/event` |
...
pnpm install:hooks    # merge 5 hooks 進 ~/.Codex/settings.json（idempotent，會備份原檔）
pnpm start            # daemon
pnpm dev              # tsx watch
pnpm test             # vitest
pnpm typecheck
...
- 跨視窗 focus 是近似的：`vscode://anthropic.Codex/open?session=<uuid>` 只會打到目前焦點視窗（Anthropic 設計）。Phase 1.5 計畫用 Win32 `FindWindow` + `SetForegroundWindow`。
...
PS hooks ──POST /event──▶  daemon (127.0.0.1:8765)  ──WS /ws──▶  web dashboard
                                │                  ──WS /ws_ext──▶ miki-helper (VSCode ext) ──/send──▶ Codex panel
