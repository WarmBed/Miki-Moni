...
| `git add -A` while there's untracked stuff from parallel sessions | accidentally commits `.Codex/worktrees/feat+codex-support`, smoke-test scripts, debug PNGs | always `git add <specific files>` |
| `npm i -g miki-moni@latest` while `miki Codex` panel still open | EBUSY locking `Codex.exe` (Anthropic SDK binary) | ask user to Ctrl+C every wrap process first |
