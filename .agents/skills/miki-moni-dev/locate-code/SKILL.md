...
Navigation reference for the miki-moni codebase (a.k.a. cc-hub). Each row points at the file + function where a feature lives, so future Codex can jump straight in instead of grepping from zero.
...
| Wrapped CLI | `src/cli/wrap.ts` | spawned per session by `miki Codex` |
...
| CLI | `bin/miki.mjs` → `src/cli/miki.ts` | subcommands: `start setup pair Codex install-hooks` |
...
| `/send` (push prompt to wrapped Codex) | `src/server.ts:558` — wrap-push fast path + VSCode prefill fallback |
...
| Install hooks into `~/.Codex/settings.json` | `pnpm install:hooks` → `src/cli/install-hooks.ts` |
...
| Per-session JSONL transcripts | `~/.Codex/projects/<encoded-cwd>/<uuid>.jsonl` (Codex Agent SDK manages) |
